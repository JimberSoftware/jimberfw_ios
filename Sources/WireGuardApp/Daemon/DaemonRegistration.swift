...
func register(userAuthentication: UserAuthentication, daemonName: String) async throws -> NetworkIsolationDaemon {
    do {
        ...
        let createdDaemon = try await createDaemon(userId: userId, company: companyName, daemonData: createDaemonData)
        ...
        let customError = NSError(domain: "JimberNetworkIsolation", code: -1, userInfo: [NSLocalizedDescriptionKey: error.localizedDescription])
        throw customError;
    }
}
